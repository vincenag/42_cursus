...
/*   Updated: 2023/12/19 10:41:21 by agvincen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_check_meals(t_philo *philo)
{
    ...
    if (philo->table->nb_meals != -1 && philo->x_eaten >= philo->table->nb_meals)
    {
...
void ft_check_status(t_table *table, t_philo *philos)
{
    ...
    while (1)
    {
        ...
        if (table->nb_meals != -1 && table->x_meals >= table->nb_meals)
        {
        ...
        if (ft_get_time() - philos[i].last_meal > (uint64_t)table->time_to_die)
        {
            ...
            printf("%lu %d died\n", ft_get_time() - table->start_time, philos[i].id);
            break;
        }
